...
public class RxAppTool {
    ...
    private static Thread.UncaughtExceptionHandler restartHandler = new Thread.UncaughtExceptionHandler() {
        public void uncaughtException(Thread thread, Throwable ex) {
            //发生崩溃异常时,重启应用
            Context myContext = RxTool.getContext();
            Intent intent = myContext.getPackageManager()
                    .getLaunchIntentForPackage(myContext.getPackageName());
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_CLEAR_TASK);
            intent.putExtra("app", 1);
            PendingIntent restartIntent = PendingIntent.getActivity(myContext, 0, intent, PendingIntent.FLAG_ONE_SHOT);
            //重启应用
